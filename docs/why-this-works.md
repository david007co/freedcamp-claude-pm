...
New ideas trigger dopamine. Returning to an existing task — especially one that has gotten
hard or boring — does not. This explains the classic pattern:
...
overwhelm — the system itself becomes an obstacle. High standards for organization + limited
time to maintain = a system that gets built but never used.
...
a commitment — it becomes another list. The cap is the feature, not a limitation.
...
causing sync problems — one side gets updated while the other stays stale. The pointer
